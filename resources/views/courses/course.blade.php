            ...
            <div class="flex overflow-x-auto pb-2 md:pb-0 space-x-3 w-full scrollbar-hide">
                @foreach ($displayCategories as $category)
                    ...
                    <button type="button" 
                            ...
                            {{ $isActive ? 'border-indigo-600 bg-indigo-600 text-white shadow-md' : 'border-gray-300 bg-white text-gray-600 hover:border-gray-400 hover:text-gray-800 hover:bg-gray-50' }}">
                        {{ $category }}
                    </button>
                @endforeach
            </div>
        ...
        @if (count($courses) > 0)
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                @foreach ($courses as $course)
                    @php
                        // 🔥 Hanya untuk mengecek apakah perlu menampilkan Badge "Dimiliki"
                        $isEnrolled = isset($enrolledCourseIds) && in_array($course['id'], $enrolledCourseIds);
                    @endphp

                    {{-- 🔥 SEMUA KARTU MENGARAH KE ETALASE (DETAIL) --}}
                    <a href="{{ route('courses.show', $course['slug']) }}" class="group flex flex-col bg-white border border-gray-200 rounded-xl overflow-hidden hover:border-indigo-300 hover:shadow-lg transition duration-300 h-full relative">
                        
                        <div class="relative h-48 w-full bg-gray-100">
                            <img src="{{ $course['thumbnail'] ?? 'https://placehold.co/600x400/f8fafc/94a3b8?text=Code+Verse' }}" alt="Thumbnail {{ $course['title'] }}" class="w-full h-full object-cover">
                        ...
                        <div class="p-5 flex-1 flex flex-col border-t border-gray-50">
                            ...
                            <div class="mt-auto pt-4 border-t border-gray-100 flex justify-between items-center">
                                <div>
                                    @if ($isEnrolled)
                                        <span class="text-xs font-bold text-teal-600 bg-teal-50 px-2.5 py-1.5 rounded-md border border-teal-100">✔ Dimiliki</span>
